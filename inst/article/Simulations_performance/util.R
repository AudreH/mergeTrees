...
oneRun <- function(sim_label, dataSets, reference, score = NID){
  ...
  # time_rScDC <- system.time(rScDC_res <- directClustering(dataSets_rspectral4))[3] + time_rSVD4
  
  # # Spectral Average Distance Clustering
  # time_rScAD <- system.time(rScAD_res <- averagedClustering(dataSets_rspectral4))[3] + time_rSVD4
  ...
  # NID_SpDC       <- apply(cutree(rScDC_res  , 1:nb_ind), 2, score, c2 = reference)
  # NID_SpAD       <- apply(cutree(rScAD_res  , 1:nb_ind), 2, score, c2 = reference)
  ...
  do.call(rbind, 
          list(
            MTW  =     data.frame(nb_grp=1:nb_ind, Sim=sim_label, method="MC"      , NID=NID_MTW      , time = time_MTW),
            ...
            # SpDC = data.frame(nb_grp=1:nb_ind, Sim=sim_label, method="SpDC"      , NID=NID_SpDC       , time = time_rScDC),
            # SpAD = data.frame(nb_grp=1:nb_ind, Sim=sim_label, method="SpAD"      , NID=NID_SpAD       , time = time_rScAD),
            rScMTW2 =  data.frame(nb_grp=1:nb_ind, Sim=sim_label, method="SpMC_2"  , NID=NID_rScMTW2  , time = time_rScMTW2),
            rScMTW4 =  data.frame(nb_grp=1:nb_ind, Sim=sim_label, method="SpMC_4"  , NID=NID_rScMTW4  , time = time_rScMTW4)
          ))

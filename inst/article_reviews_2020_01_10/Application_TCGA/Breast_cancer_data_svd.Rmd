...
15/01/20 : Re-Changement : on essai la svd tout court. 
...
```{r}
rSVD <- lapply(dataSets, svd, nu = k_svd, nv = 0) ### MODIF : pas rsvd, svd tout court.
rSVD_dataSets = lapply(rSVD, FUN = function(svd_res) svd_res$u %*% diag(svd_res$d[1:k_svd]))
```
...
```{r}
rSVD <- svd(do.call("cbind", dataSets), nu = k_svd, nv = 0) ### MODIF : pas rsvd, svd tout court.
dataSets_spectral <- as.list(as.data.frame(rSVD$u %*% diag(rSVD$d[1:k_svd])))
```

```{r}
hc_list_methods$ScDC = hclust(dist(as.data.frame(rSVD$u %*% diag(rSVD$d[1:k_svd])), method = "euclidean"), method = "ward.D2")
...
```{r}
rSVD <- lapply(dataSets, svd, nu = k_svd)
rSVD_dataSets = lapply(rSVD, FUN = function(svd_res) svd_res$u%*% diag(svd_res$d[1:k_svd]))

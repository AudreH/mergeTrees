...
res_NID = mclapply(1:length(samples_numbers), FUN = function(indice_subset){
  ...
  rSVD_dataSets = lapply(lapply(dataSets_subset, svd, nu = k_svd, nv = 0), FUN = function(svd_res) svd_res$u[,1:k_svd]%*% diag(svd_res$d[1:k_svd]))
  ...
  rSVD <- svd(do.call("cbind", dataSets_subset), nu = k_svd, nv = 0) 
  ds_sp_concat_univar <- as.list(as.data.frame(rSVD$u[,1:k_svd] %*% diag(rSVD$d[1:k_svd])))
  ...
  hc_list_methods$ScDC = hclust(dist(as.data.frame(rSVD$u[,1:k_svd] %*% diag(rSVD$d[1:k_svd])), method = "euclidean"), method = "ward.D2")
...
# save(res_NID, file = "res_NID_center_k_svd_5_20_01_20.RData")
...
```{r}
load("res_NID_center_k_svd_5.RData")
```

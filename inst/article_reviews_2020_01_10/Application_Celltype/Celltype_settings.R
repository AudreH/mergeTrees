### CELLTYPE DIVISION PAR PREMIERE VALEUR PROPRE

# 27/09/19
# en fait, division par la première valeur singulière !

library(tidyverse)
library(devtools)
library(mergeTrees)
library(rsvd)
# library(mixOmics) # pour nearZeroVar, pas accessible depuis caret puisque pas installable.
...
setwd("/travail/ahulot/Celltype_2020_01_15/")
...
rSVD <- lapply(dataSets, svd, nv = 0) # MODIF : svd et pas rsvd
rSVD_dat3 = svd(dat3, nv = 0) # MODIF : svd et pas rsvd
...
for(i in 2:20){
  ...
  wd_var = paste0("/travail/ahulot/Celltype_2020_01_15/k_svd_", k_svd, "/")
  dir.create(wd_var, showWarnings = FALSE, recursive = TRUE)
  
  setwd(wd_var)
  
  # ---- Reconstitution de la table : -----
  
  rSVD_dataSets = lapply(rSVD, FUN = function(svd_res) svd_res$u[,1:k_svd]%*% diag(svd_res$d[1:k_svd]))
...
#   save.image(file='Session1.RData')
  ...
  # --- Spectral (application en univariee) : -----
  
  dataSets_spectral <- as.data.frame(rSVD_dat3$u[,1:k_svd] %*% diag(rSVD_dat3$d[1:k_svd]))

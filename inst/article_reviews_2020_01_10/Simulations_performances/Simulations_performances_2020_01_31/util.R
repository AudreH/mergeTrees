...
#' @param n1 nombre de jeux de données avec information # Nombre de vecteurs tout court
#' @param n_grp  nombre de groupe
#' @param grp_size  taille des groupes
#' @param separability coefficient de séparabilité des groupes
scenario1 <- function(n1, n_grp, grp_size, separability){
  ...
  grp_levels <- rep(separability, n1)
...
scenario2 <- function(n1, n_grp, n_grp_per_data, grp_size, separability){
  ...
  means = rep(grp_means, grp_means_sizes)
  
  # Chaque groupe a la même moyenne, sur toute la table
  # Tous les vecteurs de la table portent les mêmes groupes
  # Seul le bruit change. 

...
setwd("/home/hulot/Documents/packages_R/mergeTrees/inst/article_reviews_2020_01_10/Simulations_performances/Simulations_performances_2020_01_28/")
...
n_tables_p = 3
n_tables_info_p = c(3)
# n_tables_info_p = c(2,3)
n_grp_p = c(5,10)
grp_size_p = c(10, 25,50)
# n0_p = c(100, 250, 500) 
# n1_p = c(50, 100, 250) 
n1_p = c(500, 550)
n_grp_per_data_p = c(2,3)
...
# grid_param = grid_param[-which(grid_param$ngrpData>=grid_param$Grp),]
# grid_param = grid_param[-which(grid_param$n1>grid_param$n0),]
...
Run_func =  function(dataSets, n_grp, grp_size, reference){
  ...
  # hc_methods$spAD_6 = averagedClustering(dat_univar)
  # hc_methods$spMC_6 = mergeTreesWard1d(dat_univar)
  # hc_methods$spDC_6 = directClustering(dat_univar)
...
for(i in 1:nrow(grid_param)){
  ...
  sep_par = c(0.2, 0.4, 1)
  res_scen = lapply(sep_par, FUN = function(sep){function_sim(n_tables, n1, n_grp, grp_size, sep, n_grp_per_data, n_eval = 2, reference, ntab_info = n_tab_info)})
  ...
  p_all
  ggsave(paste0("_n1=", n1, "_ngroups=", n_grp, "_grp size=", grp_size,
                "_ntab=",n_tables, "_ngrp_per_data=", n_grp_per_data, "_n_info=" , n_tab_info, ".jpeg"), height = 10, width = 25, units = c("cm"))  
}
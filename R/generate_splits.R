...
orderRules <- function(listRules){
  ...
  allLambdaRules = unlist(lapply(1:length(listRules), FUN = function(x){return(listRules[[x]]$lambda.rules)}))
  allRulesIndex = allRulesIndex[order(allLambdaRules, decreasing = TRUE),]
  return(allRulesIndex)
  # return(allRulesIndex[order(sapply(listRules, FUN=function(x) x$lambda.rules), decreasing=TRUE), ])
}
